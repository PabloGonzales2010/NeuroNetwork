...
///Класс, задающий перцептрон (нейросеть)
...
class Perceptron {
    ...
    void back(const std::vector<double>& y) {
        allErrors = empty;
...
public:
    ...
    ///Конструтор перцептрона
    Perceptron(const std::vector<int>& layers) {
        learningRate = 0.05;
        success = 0;
        all = 0;
        successrate = 0;
...
///Функция для обработки входного csv файла
...
int main()
{
    ...
    std::string end;
    std::cout << "Для выхода введите любой символ и нажмите Enter...\n";
    std::cin >> end;
}
